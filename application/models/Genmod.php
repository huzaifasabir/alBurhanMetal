...
class Genmod extends CI_Model{
    ...
    */
     ...
     * @return boolean
     */
    public function addevent($event, $eventItemCodeOrRef, $eventDesc, $eventTable, $staffId, $itemName = ""){
        $data = ['event'=>$event, 'eventItemCodeOrRef'=>$eventItemCodeOrRef, 'eventDesc'=>$eventDesc, 'eventTable'=>$eventTable, 'staffInCharge'=>$staffId, 'itemName' => $itemName];
    ...
    */
    ...
    public function getYearEarnings($year=""){
        ...
        if($this->db->platform() == "sqlite3"){
			$q = "SELECT transDate, totalPrice FROM transactions1 WHERE strftime('%Y', transDate) = '{$year_to_fetch}'";
