...
class Logs extends CI_Controller{
    ...
    public function index(){
        $data['pageContent'] = $this->load->view('logs/logs', '', TRUE);
    ...
    */
    ...
    public function lilt(){
        ...
        $config = $this->genlib->setPaginationConfig($totalItems, "logs/lilt", $limit, ['onclick'=>'return lilt(this.href);']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all items from db
        $data['allLog'] = $this->elog->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalItems > 0 ? "Showing " . ($start+1) . "-" . ($start + count($data['allLog'])) . " of " . $totalItems : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        //$data['cum_total'] = $this->item->getItemsCumTotal();
        
        $json['logListTable'] = $this->load->view('logs/loglisttable', $data, TRUE);//get view with populated items table
        
        //$json['itemsListTable'] = 1;
