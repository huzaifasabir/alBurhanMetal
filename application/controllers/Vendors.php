...
class Vendors extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(['vendor','AccountTransaction']);
    }
    ...
    public function index(){
        $data['pageContent'] = $this->load->view('vendors/vendors', '', TRUE);
    ...
    public function addVendor(){
        $data['pageContent'] = $this->load->view('vendors/addVendor', '', TRUE);
        $data['pageTitle'] = "Vendors";
    ...
    public function addTransaction(){
        ...
        $data['pageContent'] = $this->load->view('vendors/addTransaction', $transData, TRUE);
        $data['pageTitle'] = "Vendors";
    ...
    */
    ...
    public function add(){
        ...
        $this->form_validation->set_rules('companyName', 'Company name', ['required', 'trim', 'max_length[80]','is_unique[vendors.companyName]'],
                ['required'=>"required"]);
        $this->form_validation->set_rules('salesRepresentative', 'Sales Representative', ['required', 'trim', 'max_length[80]'],
                ['required'=>"required"]);
        $this->form_validation->set_rules('email', 'Email', ['required', 'trim', 'max_length[80]'],
                ['required'=>"required"]);
        $this->form_validation->set_rules('phone', 'Phone', ['required', 'trim', 'max_length[80]'],
                ['required'=>"required"]);
        $this->form_validation->set_rules('address', 'Address', ['required', 'trim', 'max_length[80]'],
                ['required'=>"required"]);
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $insertedId = $this->vendor->add(set_value('companyName'), set_value('salesRepresentative'), set_value('email'),set_value('phone'), 
                    set_value('address'),set_value('outstandingBalance'));
            ...
            $desc = "Addition of new vendor, {$companyName}";
            //$json['msg'] = "hello2";
            
            
            $insertedId ? $this->genmod->addevent("Creation of new Vendor", $insertedId, $desc, "vendors", $this->session->admin_id) : "";
            ...
            $json = $this->db->trans_status() !== FALSE ? 
                    ['status'=>1, 'msg'=>"Vendor successfully added"] 
                    : 
                    ['status'=>0, 'msg'=>"Oops! Unexpected server error! Please contact administrator for help. Sorry for the embarrassment"];
                  
            //$this->db->trans_complete();
            //$json['msg'] = "One or more required fields are empty or not correctly filled hello2";
            //$json['status'] = 0;   
            ...
            $json['msg'] = "One or more required fields are empty or not correctly filled hello1";
            $json['status'] = 0;
        }
    ...
    */
    ...
    public function addNewTransaction(){
        ...
        $amount = $this->input->post('amount', TRUE);//vat percentage
        $transDate = $this->input->post('transDate', TRUE);//discount percentage

        $this->db->trans_start();
        
        $dataAccountTransaction = ['accName'=>$vendor,  'debitAmount'=>$amount, 'type'=>'vendor', 'description'=>'vendor payment', 'installmentDate'=>$transDate];
        $this->AccountTransaction->addTransaction($dataAccountTransaction );
        $this->vendor->decrementBalance($vendor, $amount, $transDate );

        $this->db->trans_complete();

        $json = $this->db->trans_status() !== FALSE ? 
                    ['status'=>1, 'msg'=>"Transaction successfully added"] 
                    : 
                    ['status'=>0, 'msg'=>"Oops! Unexpected server error! Please contact administrator for help. Sorry for the embarrassment"];
