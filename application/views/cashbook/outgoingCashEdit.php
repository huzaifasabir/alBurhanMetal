...
if(isset($vendors) && !empty($vendors)){    
    foreach($vendors as $get){
        $current_vendors[$get->companyName] = [$get->companyName,$get->outstandingBalance];
    }
}
$current_payable = [];
...
$current_shopExpenseCategory = [];

if(isset($shopExpenseCategory) && !empty($shopExpenseCategory)){    
    foreach($shopExpenseCategory as $get){
        $current_shopExpenseCategory[$get->category] = $get->category;
    }
}
$current_homeExpenseCategory = [];

if(isset($homeExpenseCategory) && !empty($homeExpenseCategory)){    
    foreach($homeExpenseCategory as $get){
        $current_homeExpenseCategory[$get->category] = $get->category;
    }
}
?>
<script>
    ...
    var currentshopExpenseCategory = <?=json_encode($current_shopExpenseCategory)?>;
    var currenthomeExpenseCategory = <?=json_encode($current_homeExpenseCategory)?>;
</script>
    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="col-sm-10 " id='createNewItemDiv'>
                <div class="well">
                    <h1>Outgoing Cash Transaction</h1>
                    <button class="close cancelAddItem">&times;</button><br>
                    <form name="addNewItemForm" id="addNewItemForm" role="form" method="post" action="<?php echo site_url()?>Cashbook/editOutgoingPost">
                        ...
                        <br>
                        <div class="row">
                            <div class="col-sm-6 form-group-sm"></div>
                            <div class="col-sm-6 form-group-sm">
                            <h3><span class="help-block" id="cashBalance">0</span></h3>
                        </div>
                        </div>
                        ...
                        <div class="row" id="categoryDiv">
                            <div class="col-sm-12 form-group-sm">
                                ...
                                <select id="category" name="category" class="form-control cashCategory" required>
                                    ...
                                    <option value="shopExpense">Shop Expense</option>
                                    <option value="homeExpense">Home Expense</option>
                                    <option value="hardwareExpense">Hardware Expense</option>
                                    <option value="golaExpense">Gola Expense</option>
                                    <option value="pvcDoor">Pvc Door Expense</option>
                        ...
                        <div class="row" id="toDiv">
                            <div class="col-sm-12 form-group-sm">
                                <label for="to">To</label>
                                <select id="to" name="to" class="form-control cashTo" required>
                                ...
                                <span class="help-block errMsg" id="toErr"></span>
                            </div>
                        </div>
                        ...
                        <div class="row" id="amountDiv">
                            <div class="col-sm-12 form-group-sm">
                                ...
                                <input type="text" id="amount" name="amount" placeholder="Amount in Rs"
                                    class="form-control amountRs" min="0" required value="<?php echo $transaction[0]->debitAmount ?>" onchange="checkAmountField()">
...
<!--end of modal-->
<script src="<?=base_url()?>public/js/cashbook/outgoingCash.js"></script>