...
$(document).ready(function(){
    ...
    document.getElementById('date1').value = transInfo[0]['transDate'].split(" ")[0];
        ...
        for(let key in transInfo){
                    ...
                    var thickness = 0;
                    //console.log(transInfo[key]['itemCode']);
                    ...
                    cloned.find(".itemThickness").html(transInfoArray[transId][1]);
                    //console.log(thickness);
                    //console.log(transInfo);
                    cloned.find(".itemTransQty").val(transInfo[key]['quantity']);
                    cloned.find(".itemTotalPrice").html(transInfo[key]['totalPrice']);
                    cloned.find(".itemFixedPrice").val(transInfo[key]['fixedCostPrice']);
                    cloned.find(".itemCostPrice").html(transInfo[key]['costPrice']);
                    
                    //console.log(transInfo[key]['fixedCostPrice']);
                    ...
                    $('#transportCost').val(parseInt(transInfo[0]['transport'])).change();
            $('#laborCost').val(parseInt(transInfo[0]['labor'])).change();
                    ceipacp();
    ...
    $("#confirmPurchaseOrder").click(function(){
        ...
        if( (!vendor || !date1 || vendorPayable === '0' || cumAmount === '0')){
            ...
            return new Promise(function(resolve, reject){
                ...
                $.ajax({
                    url: appRoot+"items/editPurchaseOrder",
                    method: "post",
                    data: {transRef:transInfo[0]['ref'],_aoi:_aoi,  _ca:cumAmount, vendor:vendor,  transportCost:transportCost, laborCost:laborCost,vendorPayable:vendorPayable, date1:date1},
