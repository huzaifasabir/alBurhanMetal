...
$(document).ready(function(){
    ...
    $("#receiptDiv").on("change", ".selectedReceiptDefault", function(e){
       ...
       // console.log(ref);
        ...
        $.ajax({
            url: appRoot+"items/transactionInfo",
            type: "get",
            data: {ref:ref},
    ...
    $("#confirmPurchaseOrder").click(function(){
        ...
        if( (!vendor || !date1 || vendorPayable === '0' )){
            !vendor ? $("#vendorErr").text("required") : "";
            ...
            changeInnerHTML(["vendorErr", "vendorPayableErr","date1Err"], "");
            ...
            for(var i = 0; i < selectedItemNodeLength; i++){
                ...
                var itemPurchaseQtyElem = selectedItemNode[i].parentNode.parentNode.children[1].children[1];
                var itemUnitPriceElem = selectedItemNode[i].parentNode.parentNode.children[2].children[1];
                var qtyNeededElem = selectedItemNode[i].parentNode.parentNode.children[3].children[1];
                var totalPriceElem = selectedItemNode[i].parentNode.parentNode.children[4].children[1];
                ...
                //get values
                //var availQty = parseInt(itemPurchaseQtyElem.innerHTML);
                var qty = parseInt(qtyNeededElem.value);
                //var unitFixedPrice = parseInt(unitFixedPriceNode.value);
                var unitPrice = parseInt(itemUnitPriceElem.innerHTML);
                //var sellingPrice = parseFloat(sellingPriceNode.value);
                var totalPrice = (totalPriceElem.innerHTML);
                //var expectedTotPrice = +(unitPrice*qty).toFixed(2);
                //validate data
                if((qty === 0) ){
                    //totalPriceNode.style.backgroundColor = expectedTotPrice !== totalPrice ? "red" : "";
                    qtyNeededElem.style.backgroundColor = (qty === 0) ? "red" : "";   
                    return;
                }
            ...
            return new Promise(function(resolve, reject){
                ...
                $.ajax({
                    url: appRoot+"items/returnOrder",
                    method: "post",
                    data: {_aoi:_aoi,receiptNo:receiptNo, vendor:vendor, vendorPayable:vendorPayable, date1:date1},
...
function ceipacp(){
    ...
    $(".transItemList").each(function(){
        ...
        //var totalPrice = parseInt($(this).find(".totalCostPrice").html());

        var itemPrice = parseInt($(this).find(".itemPrice").html());
        
        //current item's quantity to be sold
        //var transQty = parseInt($(this).find(".itemTransQty").val());
        
        var totalPrice =  itemPrice * transQty;
    ...
    $("#vendorPayable").html(addCommas(totalAmount));
...
function selectedItem(selectedNode){
    if(selectedNode){
        ...
        var itemPurchaseQtyElem = selectedNode.parentNode.parentNode.children[1].children[1];
        ...
        itemPurchaseQtyElem.innerHTML = products[itemCode]['quantityPurchased'];
        itemUnitPriceElem.innerHTML = products[itemCode]['unitPrice'];
        qtyNeededElem.value = 1;
        
        totalPriceElem.innerHTML = products[itemCode]['unitPrice'];
        ceipacp();
...
/*
CREATE TABLE `returnPurchase` (
  `transId` bigint(20) UNSIGNED NOT NULL,
  `ref` int(12) NOT NULL,
  `referenceRef` int(12) NOT NULL,
