...
$(document).ready(function(){
			...
			return new Promise((res, rej)=>{
				...
				for(let key in products){
					...
					if(!inArray(key, selectedItemsArr)){
						...
						cloned.find(".selectedItemDefault").append("<option value='"+key+"'>"+products[key]['name']+"</option>");
					}
				}
    ...
    $("#receiptDiv").on("change", ".selectedReceiptDefault", function(e){
        e.preventDefault();
        var ref = $("#receiptNo").val();
       // console.log(ref);
        //$.get(appRoot + "/transactions/transactionInfo", {ref:ref}, function (data) {  alert(data);             });
        
        $.ajax({
            url: appRoot+"transactions/transactionInfo",
            type: "get",
            data: {ref:ref},
            success: function(returnedData){
                if(returnedData.status === 1){
                    //console.log("here");
                    //console.log(returnedData.customerI);
                    //console.log(returnedData.productI);
                    var productI = returnedData.productI;
                    products = returnedData.productI;
                    //console.log(products);
                    //for(let key in productI){

                    //    console.log(productI[key]['name']);

                    //}
                    var info = returnedData.customerI;
                    //console.log(info);

                    //console.log(info['customerName']);                    
                    $("#custName").html(info['name']);
                    $("#custPhone").html(info['phone']);
                    $("#custEmail").html(info['email']);
                    $("#custAddress").html(info['address']);
                    $("#previousBalance").html(info['outstandingBalance']);
                    $("#invoiceTotal").html("");
                    resetTransList();
                    ceipacp();
    ...
    $("#confirmSaleOrder").click(function(){
        ...
        var profitReduction = parseInt($("#totalProfit").html());
        var remainingBalance = parseInt($("#remainingBalance").html());
        var amountPayable = parseInt($("#amountPayable").html());
        var date1 = $("#date1").val();
        
        //console.log(receiptNo);
        if(receiptNo === '0'){
            $("#receiptNoErr").html("Required");
            return;
        }else{$("#receiptNoErr").html("");}
        ...
        if(invoiceTotal === 0){
            ...
            for(var i = 0; i < selectedItemNodeLength; i++){
                var itemName = selectedItemNode[i].value;
                itemName = products[itemName]['name'];
                ...
                var costUnitPriceNode = selectedItemNode[i].parentNode.parentNode.children[6].children[1];
                ...
                var unitPrice = parseInt(unitPriceNode.innerHTML);
                var totalPrice = parseInt(totalPriceNode.innerHTML);
                ...
                if((qty === 0) || (availQty < qty) || (expectedTotPrice !== totalPrice)){
                    ...
                    var itemInfo = {_iN:itemName, qty:qty, unitPrice:unitPrice, costUnitPrice:costUnitPrice, totalPrice:totalPrice};
            ...
            return new Promise(function(resolve, reject){
                ...
                $.ajax({
                    url: appRoot+"transactions/returnOrder",
                    method: "post",
                    data: {_aoi:_aoi, remainingBalance:remainingBalance, previousBalance:previousBalance, _ca:cumAmount,
                        profitReduction:profitReduction, invoiceTotal:invoiceTotal, amountPayable:amountPayable, receiptNo:receiptNo, cn:custName , date1:date1},
...
function ceipacp(){
    ...
    $(".transItemList").each(function(){
        ...
        //else{//if all is well
            ...
            var unitProfit = parseFloat($(this).find(".itemUnitPrice").html() - $(this).find(".costUnitPrice").html() );
    ...
    $("#totalProfit").html(Math.round(totalProfitTransaction));
    calchadue();
...
function selectedItem(selectedNode){
    if(selectedNode){
        console.log(products);
        var itemName = selectedNode.value;
        //console.log(itemName);
        ...
        var costPriceElem = selectedNode.parentNode.parentNode.children[6].children[1];
        //console.log(costPriceElem);
        itemAvailQtyElem.innerHTML = products[itemName]['quantityPurchased'];
        itemUnitPriceElem.innerHTML = products[itemName]['unitPrice'];
        qtyNeededElem.value = 1;
        
        totalPriceElem.innerHTML = products[itemName]['unitPrice'];
        costPriceElem.innerHTML = products[itemName]['costUnitPrice'];
        console.log(products[itemName]['costUnitPrice']);
        ceipacp();
        calchadue();
        ...
        $.ajax({
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    itemUnitPriceElem.innerHTML = parseFloat(returnedData.sellingPrice);
                    costUnitPriceElem.innerHTML = parseFloat(returnedData.costPrice);
                    qtyNeededElem.value = 1;
...
function calchadue(){
    ...
    if(cumAmount < 0){
        //console.log("true");
        remainingBalance = -(cumAmount);
        $("#remainingBalance").html(remainingBalance);
        $("#amountPayable").html(0);
        return;    
    }
    else{
        $("#remainingBalance").html(0);
        $("#amountPayable").html(cumAmount);
        return;
    }
...
function loadReceipts(){
    ...
    for(let key in receiptsArray){
                    
                //console.log(key);    
                        //if the item has not been selected, append it to the select list
                        $("#receiptNo").append("<option value='"+key+"'>"+receiptsArray[key]+"</option>");
