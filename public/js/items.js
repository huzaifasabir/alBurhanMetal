...
$(document).ready(function(){
    ...
    $("#category").change(function(){
        ...
        if(category){
            $.ajax({
                url: appRoot+"search/itemSearchCategory",
                type: "get",
                data: {category:category,flag:flag},
                success: function(returnedData){
        ...
        $.ajax({
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    console.log("here");
                    $("#itemNameErr").text(returnedData.error);
    ...
    $("#itemSearch").change(function(){
     ...
     $("#itemSearchAll").change(function(){
        ...
        if(value){
            $.ajax({
                url: appRoot+"search/itemSearchAll",
                type: "get",
                data: {v:value, category:category},
                success: function(returnedData){
                    $("#itemsListTable").html(returnedData.itemsListTable);
                }
            });
...
function printWithoutCost() {
  var element = document.getElementById("costPriceHeader");
  var element1 = document.getElementsByClassName("costPriceRow");
for (var item of element1) {
    //console.log(item);
    item.classList.add("hidden-print");
}
  element.classList.add("hidden-print");

  var element = document.getElementById("totalValueHeader");
  var element1 = document.getElementsByClassName("totalValueRow");
for (var item of element1) {
    //console.log(item);
    item.classList.add("hidden-print");
}
...
function loadCategories(){
    ...
    for(let key in currentCategories){
                ...
                //console.log(key);    
                        //if the item has not been selected, append it to the select list
                        $("#category").append("<option value='"+key+"'>"+currentCategories[key]+"</option>");
