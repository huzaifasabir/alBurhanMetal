...
$(document).ready(function(){
    ...
    //latr_();
    //console.log(transInfoArray);
    //console.log(transInfo);
    //loadCustomers();
    //console.log(customersArray);
    document.getElementById('date1').value = transInfo[0]['transDate'].split(" ")[0];
        ...
        $.ajax({
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    $("#previousBalance").html(parseInt(info['outstandingBalance']) + parseInt(transInfo[0]['previousBalance']) - parseInt(transInfo[0]['remainingBalance'])) ;
                    //console.log(parseInt(info['outstandingBalance']) + parseInt(transInfo[0]['previousBalance']) - parseInt(transInfo[0]['remainingBalance']));
                    //console.log(info['outstandingBalance']);
                    //console.log(transInfo[0]['invoiceTotal']);
                    //console.log(transInfo[0]['amountTendered']);
                    //console.log(transInfo[0]['changeDue']);
                    ceipacp();
        ...
        for(let key in transInfo){
                    
                        //console.log(key);    
                        //if the item has not been selected, append it to the select list
                        
                        var cloned = $("#divToClone").clone();
                    var id  = transInfo[key]['itemCode'];
                    var transId = transInfo[key]['transId']
                    cloned.addClass('transItemList').removeClass('hidden').attr('id', '');
                    
                    //reset the form values (in the cloned div) to default
                    //console.log('hello');
                    cloned.find(".selectedItemDefault").addClass("selectedItem").val(key);
                    cloned.find(".itemAvailQty").html(transInfoArray[transId][0]);
                    cloned.find(".itemUnitPrice").val(transInfo[key]['unitPrice']);
                    cloned.find(".itemTransQty").val(transInfo[key]['quantity']);
                    cloned.find(".itemTotalPrice").html(transInfo[key]['totalPrice']);
                    cloned.find(".costUnitPrice").html(transInfo[key]['costUnitPrice']);
                    cloned.find(".totalCostPrice").html(transInfo[key]['totalCostPrice']);
                    cloned.find(".selectedItemDefault").prepend("<option value='"+id+"' selected>"+currentItems[id]+"</option>");
            ...
            $('#discountValue').val(parseInt(transInfo[0]['discount_amount'])).change();

            //var discountPercentage = (parseInt(transInfo[0]['discount_amount'])/transInfo[0]['invoiceTotal']) * 100;

        //display the discount (%)
            //$("#discount").val(discountPercentage).change();
            //$('#discount').val(parseInt(transInfo[0]['discount_amount']));            }
            $('#amountTendered').val(transInfo[0]['amountTendered']);
            
            //ceipacp();
            //console.log(transInfo[0]['discount_amount']);
    ...
    $("#amountTendered").on('change focusout keyup keydown keypress', function(e){
        //$("#balance").html("");
        var amountTendered = ($("#amountTendered").val());

        $("#amountTendered").val(addCommas(amountTendered));
        calchadue();
    ...
    $("#confirmSaleOrder").click(function(){
        ...
        if(!newCustomer && !custId){
            $("#newTransErrMsg").html("Select One New Customer or From dropdown");
            return;
        }
        else{$("#newTransErrMsg").html("");}
        ...
        if(invoiceTotal === 0){
            ...
            for(var i = 0; i < selectedItemNodeLength; i++){
                ...
                var totalPrice = (totalPriceNode.innerHTML);
                var c = totalPrice.split(',');
                var d = c.join('');
                totalPrice =  parseInt(d);
                var totalCostPrice = (totalCostPriceNode.innerHTML);
                var expectedTotPrice = +(unitPrice*qty);
                var costUnitPrice = parseFloat(costUnitPriceNode.innerHTML);
                var totalProfitItem = (totalProfitItemNode.innerHTML);
                //validate data
                if((qty === 0) || (availQty < qty) || (expectedTotPrice !== totalPrice)){
                    //console.log(expectedTotPrice);
            ...
            return new Promise(function(resolve, reject){
                ...
                $.ajax({
                    url: appRoot+"transactions/editSalesOrder",
                    method: "post",
                    data: {transRef:transInfo[0]['ref'],_aoi:_aoi, _at:amountTendered, _cd:changeDue,remainingBalance:remainingBalance, previousBalance:previousBalance, _ca:cumAmount,
                        discount:discountPercentage,discountValue:discountValue, totalProfit:totalProfit, invoiceTotal:invoiceTotal, cn:custName, ce:custEmail, cp:custPhone, ca:custAddress, newCustomer:newCustomer, ci:custId, date1:date1},
                    ...
                    success:function(returnedData){
                        if(returnedData.status === 1){
                            ...
                            window.location = loaction + "../../../cashbook/dailyCashBook";
...
function ceipacp(){
    ...
    $(".transItemList").each(function(){
        ...
        //else{//if all is well
            ...
            var unitProfit = parseFloat($(this).find(".itemUnitPrice").val() - $(this).find(".costUnitPrice").html() );
            var totalProfitItem = unitProfit * transQty;

            $(this).find(".totalProfitItem").html(addCommas(totalProfitItem));
            //round to two decimal places
            ...
            $(this).find(".itemTotalPrice").html(addCommas(itemTotalPrice));
    ...
    return new Promise(function(resolve, reject){
        ...
        var discountAmount = $('#discountValue').val();
        ...
        $("#totalProfit").html(addCommas(Math.round(totalProfitTransaction)));
        //display the cumulative amount
        $("#invoiceTotal").html(addCommas(invoiceTotal));
 ...
 */
function calchadue(){
    //console.log("calchadue");
    var cumAmount = ($("#cumAmount").html());
    var c = cumAmount.split(',');
    var d = c.join('');
    cumAmount =  parseInt(d);
    
    var amountTendered = ($("#amountTendered").val());
    c = amountTendered.split(',');
    d = c.join('');
    amountTendered =  parseInt(d);
    ...
    if((amountTendered < cumAmount)){
        ...
        $("#remainingBalance").html(addCommas(remainingBalance));
        $("#changeDue").html("0");
    }

    else{ if(amountTendered){
        $("#changeDue").html(addCommas(+(amountTendered - cumAmount)));
        $("#remainingBalance").html("0");
        
        //remove error msg if any
        $("#amountTenderedErr").html("");
    }else{
        $("#remainingBalance").html(addCommas(cumAmount));
        $("#changeDue").html("0");
    }}
}
...
function loadCustomers(){
    ...
    for(let key in customersArray){
                    if(customersArray[key] !== transInfo[0]['cust_name']){
...
function getDiscountAmount(cumAmount){
    ...
    $('#discountValue').val(parseInt(transInfo[0]['discount_amount']));
    //$('#discountValue').val();
