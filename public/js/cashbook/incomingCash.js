'use strict';
var rent = ['shop1','shop2','shop3'];
//var account = ['js','locker','hbl'];
...
$(document).ready(function(){
        ...
        switch (category) {
          ...
          case  'payable':
            for(let key in currentPayable){
                $("#from").append("<option value='"+currentPayable[key][0]+"'>"+currentPayable[key][0]+"</option>");
            }
            break;
          case 'receivable':
            for(let key in currentReceivable){
                $("#from").append("<option value='"+currentReceivable[key][0]+"'>"+currentReceivable[key][0]+"</option>");
            }
            break;
          case 'rent':
            for(let key in rent){
                $("#from").append("<option value='"+rent[key]+"'>"+rent[key]+"</option>");
            }
            break;
        case 'customer':
            for(let key in currentCustomers){
                console.log(key);
                $("#from").append("<option value='"+currentCustomers[key][0]+"'>"+currentCustomers[key][0]+"</option>");
            }
            break;
          case 'gola':
            $("#from").append("<option value='gola'>gola</option>");
            break;
        case 'pvcDoor':
            $("#from").append("<option value='pvcDoor'>pvcDoor</option>");
            break;
          case 'hardware':
             $("#from").append("<option value='hardware'>hardware</option>");
            break;
          case 'petty':
            $("#from").append("<option value='petty'>petty cash</option>");
            break;
          case 'account':
            for(let key in currentAccounts){
                if(currentAccounts[key][0] !== 'Cash In Hand' && currentAccounts[key][0] !== 'factory')
                $("#from").append("<option value='"+currentAccounts[key][0]+"'>"+currentAccounts[key][0]+"</option>");
            }
            break;
          case 'factory':
            $("#from").append("<option value='factory'>factory</option>");
            break;
        }
    ...
    $("#fromDiv").on("change", ".cashFrom", function(e){
        $("#balance").html("");
        $("#amount").val("");
        var category = $("#category").val();
        if(category === 'account'){
            var from = $("#from").val();
            $("#balance").html("Balance: " + currentAccounts[from][1]);

        }
        if(category === 'customer'){
            var from = $("#from").val();
            $("#balance").html("Outstanding Balance: " + currentCustomers[from][1]);

        }
        if(category === 'receivable'){
            var from = $("#from").val();
            $("#balance").html("Outstanding Balance: " + currentReceivable[from][1]);
...
function checkAmountField(){
    ...
    if(category === 'account'){
        ...
        if(amount > currentAccounts[from][1]){
            $("#amountErr").html('Amount Exceeding Current Balance. Available Balance: '+ currentAccounts[from][1]);
            if(currentAccounts[from][1] === '0'){
                $("#amount").val("");
            }else{
                $("#amount").val(addCommas(currentAccounts[from][1]));
            }
        ...
        amount = amount.join('');
        amount = parseInt(amount);
        if(amount > currentCustomers[from][1]){
            /*
            $("#amountErr").html('Amount Exceeding Current Outstanding Balance. Outstanding Balance: '+ currentCustomers[from][1]);
            if(currentCustomers[from][1] === '0'){
                $("#amount").val("");
            }else{
                $("#amount").val(addCommas(currentCustomers[from][1]));
            }
            */
        }
        ...
        amount = amount.join('');
        amount = parseInt(amount);
        if(amount > currentReceivable[from][1]){
            //$("#amountErr").html('Amount Exceeding Current Balance. Available Balance: '+ currentReceivable[from][1]);
            //if(currentReceivable[from][1] === '0'){
            //    $("#amount").val("");
            //}else{
                $("#amount").val(addCommas(currentReceivable[from][1]));
            //}
