'use strict';
var shop_expense = ['food','bills','faqeer','pvcGola','salary','otla','tea','water','gola','hardware','other'];
...
$(document).ready(function(){
    ...
    $("#cashBalance").html("Cash in Hand Balance: " + currentAccounts['Cash In Hand'][1]);
    ...
    $("#categoryDiv").on("change", ".cashCategory", function(e){
        e.preventDefault();
        $("#balance").html("");
        $("#amount").val("");
        $("#amountErr").html('');
        ...
        //$("#customer").append("<option value='"+key+"'>"+rent[key]+"</option>");
        ...
        switch (category) {
          case '0':
            break;
          case  'payable':
            for(let key in currentPayable){
                $("#to").append("<option value='"+currentPayable[key][0]+"'>"+currentPayable[key][0]+"</option>");
            }
            break;
          case 'receivable':
            for(let key in currentReceivable){
                $("#to").append("<option value='"+currentReceivable[key][0]+"'>"+currentReceivable[key][0]+"</option>");
            }
            break;
          case 'vendor':
            for(let key in currentVendors){
                $("#to").append("<option value='"+currentVendors[key][0]+"'>"+currentVendors[key][0]+"</option>");
            }
            break;
        case 'customer':
            for(let key in currentCustomers){
                //console.log(key);
                $("#to").append("<option value='"+currentCustomers[key][0]+"'>"+currentCustomers[key][0]+"</option>");
            }
            break;
        case 'shopExpense':
            for(let key in currentshopExpenseCategory){
                $("#to").append("<option value='"+currentshopExpenseCategory[key]+"'>"+currentshopExpenseCategory[key]+"</option>");
            }
            break;
          case 'homeExpense':
            for(let key in currenthomeExpenseCategory){
                $("#to").append("<option value='"+currenthomeExpenseCategory[key]+"'>"+currenthomeExpenseCategory[key]+"</option>");
            }
            break;
            
        case 'hardwareExpense':
            $("#to").append("<option value='hardwareExpense'>hardware</option>");
            break;
        case 'golaExpense':
            $("#to").append("<option value='golaExpense'>gola</option>");
            break;
        case 'pvcDoor':
            $("#to").append("<option value='pvcDoor'>pvcDoor</option>");
            break;
          case 'petty':
            $("#to").append("<option value='petty'>petty cash</option>");
            break;
          case 'account':
            for(let key in currentAccounts){
                if(currentAccounts[key][0] !== 'Cash In Hand' && currentAccounts[key][0] !== 'factory')
                $("#to").append("<option value='"+currentAccounts[key][0]+"'>"+currentAccounts[key][0]+"</option>");
            }
            break;
          case 'factory':
            $("#to").append("<option value='factory'>factory</option>");
            break;
        }
});

    $("#toDiv").on("change", ".cashTo", function(e){
        $("#balance").html("");
        $("#amount").val("");
        $("#amountErr").html('');
        ...
        if(category === 'account'){
            var to = $("#to").val();
            $("#balance").html("Balance: " + currentAccounts[to][1]);
        }
        if(category === 'vendor'){
            var to = $("#to").val();
            $("#balance").html("Outstanding Balance: " + currentVendors[to][1]);

        }
        if(category === 'payable'){
            //console.log("payable");
            var to = $("#to").val();
            //console.log(currentPayable[to][1]);
            $("#balance").html("Outstanding Balance: " + currentPayable[to][1]);

        }
    });
    $("#amountDiv").on("change", ".amountRs", function(e){
        //$("#balance").html("");
    ...
    $("#showPurchaseForm").click(function(){
        console.log("here");
        $("#newPurchaseOrderDiv").toggleClass('collapse');
        
        if($("#newPurchaseOrderDiv").hasClass('collapse')){
            $(this).html("<i class='fa fa-plus'></i> New Purchase Order");
        }
        
        else{
            $(this).html("<i class='fa fa-minus'></i> New Purchase Order");
...
function checkAmountField(){
    ...
    var to = $("#to").val();
    if(amount > parseInt(currentAccounts['Cash In Hand'][1])){
        $("#amountErr").html('Amount Exceeding Current Cash in Hand Balance. Available Balance: '+ currentAccounts['Cash In Hand'][1]);
        if(currentAccounts['Cash In Hand'][1] === '0'){
            $("#amount").val("");
        }else{
            $("#amount").val(currentAccounts['Cash In Hand'][1]);
        }   
    }
    else{
        $("#amountErr").html('');
    ...
    if(category === 'vendor'){
        if(amount < parseInt(currentAccounts['Cash In Hand'][1])){
            if(amount > parseInt(currentVendors[to][1])){
            //    $("#amountErr").html('Amount Exceeding Outstanding Balance. Outstanding Balance: '+ currentVendors[to][1]);
            //    $("#amount").val(currentVendors[to][1]);
            }else{
                $("#amountErr").html('');
            }
        }
        else{
            if(parseInt(currentAccounts['Cash In Hand'][1]) > parseInt(currentVendors[to][1])){
                //$("#amountErr").html('Amount Exceeding Outstanding Balance. Outstanding Balance: '+ currentVendors[to][1]);
                //$("#amount").val(currentVendors[to][1]);
            }else{
                $("#amountErr").html('Amount Exceeding Current Cash in Hand Balance. Available Balance: '+ currentAccounts['Cash In Hand'][1]);
    ...
    if(category === 'payable'){
        if(amount < parseInt(currentAccounts['Cash In Hand'][1])){
            if(amount > parseInt(currentPayable[to][1])){
                //$("#amountErr").html('Amount Exceeding Outstanding Balance. Outstanding Balance: '+ currentPayable[to][1]);
                //$("#amount").val(currentPayable[to][1]);
            }else{
                $("#amountErr").html('');
            }
        }
        else{
            if(parseInt(currentAccounts['Cash In Hand'][1]) > parseInt(currentPayable[to][1])){
                //$("#amountErr").html('Amount Exceeding Outstanding Balance. Outstanding Balance: '+ currentPayable[to][1]);
                //$("#amount").val(currentPayable[to][1]);
            }else{
                $("#amountErr").html('Amount Exceeding Current Cash in Hand Balance. Available Balance: '+ currentAccounts['Cash In Hand'][1]);
